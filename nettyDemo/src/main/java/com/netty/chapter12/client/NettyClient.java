...
import com.netty.chapter10.ClientHandler;
        ...
        bootstrap
                ...
                .handler(new ChannelInitializer<SocketChannel>(){
                    ...
                    protected void initChannel(SocketChannel socketChannel) throws Exception {
                        socketChannel.pipeline().addLast(new PacketDecoder());//解码
                        socketChannel.pipeline().addLast(new LoginResponseHandler());//登录返回
                        socketChannel.pipeline().addLast(new MessageResponseHandler());//消息返回
                        socketChannel.pipeline().addLast(new PacketEncoder());//编码
                    }
                });
